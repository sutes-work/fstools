...
#define howmanyt 5
    ...
    name[0] = '0' + (char)prefix;

    for(i=1;i<len;i++){
        name[i] = charTable[random_int((int)strlen(charTable))];
    }
    name[i] = 0;

    /* PFH - Now, let's add an extension */
    rndnum = random_int(howmanyt);
    tstrlen = (int)strlen(dtypes[rndnum]);
    for (j = 1; j <= tstrlen; j++) {
...
void perr(int e, const char *s, ...) { // e is errno, and s is the string
    int p;
    va_list argp;
    struct tm *newtime;
    time_t aclock;
...
char    path[1024];
    ...
    case 0: /* read */
    case 1: /* read */
        offset = (int)random_int(30000) + (int)random_gauss(10, 100000);
        len = 1 + (int)random_gauss(20000, 100000);
    ...
    case 3: /* write */
        if(w){
            offset = (int)random_int(30000) + (int)random_gauss(10, 100000);
            len = 1 + (int)random_gauss(30000, 500000);
            block = malloc(len);
    ...
    case 7: /* sleep */ 
        if (sleepy) {
            if(!random_int(4)){
                usleep(1000000 * (unsigned)random_gauss(1, 200));
            }
        }
        break;
    case 8: /* rename */
...
const char      *root = random_root();
    ...
    // Perform open file operations
        ...
        if(!fileIsWritable(f->refFile))
            w = 0;
        w = w != 0;
        if(!cmpfileOpen(f, (char)w, nocache))
            goto errorOccured;
        l = (int)random_gauss(20, 5000);
        for(i=0;i<l;i++){
            if(!randomOpenOp(f, (char)w, id, running))
                goto errorOccured;
        }
        if(!cmpfileClose(f))
            goto errorOccured;
...
const char      *root = random_root();
        ...
        for(i1=0;i1<l1;i1++){
            l = (int)random_gauss(20, 5000);
                for(i=0;i<l;i++){
                    if(!randomUnlinkOp(f, (char)w, id, running)) 
                        goto errorOccured;
                }
        }
...
int main(int argc, char **argv)
{
    ...
    (void)closedir(dirp);
    if (dirs != 0)
        printf("--> WARNING: %s should be an EMPTY DIRECTORY (has %u entries) <-- \n", root1, dirs);
...
#ifdef __Fuchsia__
    zx_handle_t* processes = malloc(sizeof(zx_handle_t) * n);
    ...
    for (i = 0; i < n - 1; ++i) {
        sprintf(buf, "%u", i);
        if (fdio_spawn_etc(ZX_HANDLE_INVALID, FDIO_SPAWN_CLONE_ALL, "/bin/fstorture", new_argv, NULL, 0, NULL,
                           &processes[i], err_msg) != ZX_OK) {
